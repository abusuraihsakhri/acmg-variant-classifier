...
from acmg_classifier.frequency import DEFAULT_BA1_THRESHOLD, DEFAULT_PM2_THRESHOLD
...
def run_batch_table(args) -> List[dict]:
    ...
    with open(filepath, mode="r", encoding="utf-8-sig") as f:
        # Detect delimiter if comma or tab
        sample = f.read(2048)
        f.seek(0)
        ...
        for row_idx, row in enumerate(reader, start=1):
            ...
            af = float(af_val.strip()) if af_val.strip() else None
...
def main(argv=None) -> int:
    ...
    if args.splice:
        ...
        if args.format == "json":
            out = json.dumps(res.__dict__, indent=2)
        else:
            out = (
    ...
    if args.pgx:
        ...
        if args.format == "json":
            out = json.dumps(annot.__dict__, indent=2)
        else:
            out = (
    ...
    if args.domain:
        ...
        if args.format == "json":
            out = json.dumps([h.__dict__ for h in hits], indent=2)
        else:
